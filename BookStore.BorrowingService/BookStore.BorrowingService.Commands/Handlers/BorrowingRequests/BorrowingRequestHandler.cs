...
public class BorrowingRequestHandler(IConfiguration configuration, IBorrowingRequestRepository borrowingRequestRepository, ISendEndpointProvider provider) : IRequestHandler<BorrowingRequestCommand, BorrowingRequest>
{
    ...
    public async Task<BorrowingRequest> Handle(BorrowingRequestCommand request, CancellationToken cancellationToken)
    {
        ...
        var queueName = configuration["RabbitMq:Queues:Borrowing"] ?? throw new NullReferenceException("Queue name configuration is missing.");

        var endpoint = await provider.GetSendEndpoint(new Uri($"queue:{queueName}") ?? throw new NullReferenceException(typeof(BorrowingRequest).Name));
